...
# MMLU synthetic data, three difficulties = {young_children, college_students_ scientists}, two generators = {8b, 70b}
for GENERATOR in 8b 70b;
do
    for DIFFICULTY in young_children college_students scientists;
    do
        python3 convert_custom_dataset.py \
            --dataset "/network/eldar/datasets/data_gen/various_difficulties/mmlu/mmlu_${DIFFICULTY}_llama3_${GENERATOR}_instruct" --data_files "train_p*.jsonl" --splits train --no_wrap \
            --out_root "/nm/drive0/eldar/datasets/llama3_8b/seqlen8k_tokenized_wEOS_noWrap/mmlu_datagen_wllama3_${GENERATOR}_instruct_difficulty_${DIFFICULTY}" \
            --concat_tokens 8192 --tokenizer meta-llama/Meta-Llama-3-8B --compression zstd \
            --eos_text "<|end_of_text|>" --tokenizer_call_kwargs "{\"add_special_tokens\": false}"
    done
done
