...
# Wrapping = NO
export RUN_NAME=DownFixedFull_SYN8BwdiffsOpenLLM4x_SYN8Bopenmath1x_FineWebEduScore4.2_oneshot_wEOS_sp${SPARSITY}_${PRECISION}_maxseq${MAX_SEQ_LEN}_${MAX_DURATION}_cosineLR${LR}_warmup${WARMUP}_GradClip2_globalBS${GLOBAL_BS}_evalInterval${EVAL_INTERVAL}_wEOS_noWrap

composer train_sparse.py \
    yamls/pretrain/llama3_8b_alldownstreamfixedfull_SYN8BwdiffsOpenLLM4x_SYN8Bopenmath1x_FineWebEduScore4.2_GradClip2.yaml \
...
# GradClipping = YES (maybe check if 1.0 is better than 2.0)
# Wrapping = NO
export RUN_NAME=DownFixedFull_SYN8BwdiffsOpenLLM4x_SYN8Bopenmath1x_FineWebEduScore4.0_oneshot_wEOS_sp${SPARSITY}_${PRECISION}_maxseq${MAX_SEQ_LEN}_${MAX_DURATION}_cosineLR${LR}_warmup${WARMUP}_GradClip2_globalBS${GLOBAL_BS}_evalInterval${EVAL_INTERVAL}_wEOS_noWrap

composer train_sparse.py \
    yamls/pretrain/llama3_8b_alldownstreamfixedfull_SYN8BwdiffsOpenLLM4x_SYN8Bopenmath1x_FineWebEduScore4.0_GradClip2.yaml \
    model_name_or_path=${MDL} \
    max_seq_len=${MAX_SEQ_LEN} \
    max_duration=${MAX_DURATION} \
    eval_interval=${EVAL_INTERVAL} \
